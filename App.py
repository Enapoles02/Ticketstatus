...
def load_data_from_excel(uploaded_file):
    ...
    try:
        if created_col:
            df["Created"] = pd.to_datetime(df[created_col[0]], format="%m/%d/%Y %I:%M:%S %p", errors="coerce").dt.normalize()
        elif "Opened" in df.columns:
            df["Created"] = pd.to_datetime(df["Opened"], format="%m/%d/%Y %I:%M:%S %p", errors="coerce").dt.normalize()
        else:
            df["Created"] = pd.NaT
...
# El resto del contenido (KPIs, drilldowns, tablas) se completa en el próximo bloque
